...
test_step = 0 if train_mode else 1000000
...
save_path = f"./saved_models/cnn_cheese_dqn10_v2/{date_time}"
load_path = f"./saved_models/cnn_cheese_dqn10_v2/20231207135600"

# 연산 장치
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def pre_process(frame1, frame2=None):
    ...
    if frame2 is not None:
        new_frame = torch.sign(
            torch.sign(gray_transform(frame1)[:, 80:, 3:-3]) - 0.5 * torch.where(frame2 > 0, frame2, torch.tensor(0.)))
    else:
...
class DQNAgent():
    def __init__(self):
        ...
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=learning_rate)  # optimizer
        self.memory = deque(maxlen=mem_maxlen)  # replay buffer(memory)
        self.writer = SummaryWriter(save_path) if load_model == False else None  # save training info
    ...
    def append_sample(self, state, action, reward, next_state, done, action_idx):  # add sample
        self.memory.append((state, action, reward, next_state, done, action_idx))

    def train_model(self):  # q learning, loss=huber_loss
...
if __name__ == '__main__':
    ...
    DQN = DQN().to(device)  # DQN summary
    print(summary(DQN, (1, 95, 144)))
    ...
    for step in range(run_step + test_step):
        ...
        state = pre_process(state, old_state)
        env.render()
