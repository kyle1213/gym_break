...
epsilon_min = 0.01
explore_step = run_step * 0.1  # 500만중의 5퍼만 eps move with annealing = 50만
epsilon_delta = (epsilon_init - epsilon_min) / (1 * explore_step) if train_mode else 0.

# 모델 저장 및 불러오기 경로
date_time = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
save_path = f"./saved_models/full_cnn_cheese_dqn/{date_time}"
load_path = f"./saved_models/full_cnn_cheese_dqn/20231115155738"

# 연산 장치
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def frame_transform(frame1, frame2=None):
    ...
    if frame2 is not None:
        new_frame = torch.sign(
            torch.sign(gray_transform(frame1))[:, 17:, 3:-3] - torch.where(frame2 > 0, frame2, torch.tensor(0.)))
    else:
        new_frame = torch.sign(gray_transform(frame1))[:, 17:, 3:-3].float()
    return new_frame
...
class DQNAgent():
    def __init__(self):
        ...
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=learning_rate, weight_decay=0.9)
...
if __name__ == '__main__':
    ...
    for step in range(run_step + test_step):
        ...
        state = frame_transform(state, old_state)
        env.render()

        fake_reward = real_reward * 50

        if torch.max(torch.sum(state[:, :, :], dim=1)) == 8:  # bar와 ball이 겹치면==8, bar==4, ball==4, old object=-4
            fake_reward += 1
        if torch.min(torch.sum(state[:, :, :], dim=1)) == -8:
            fake_reward += 1
