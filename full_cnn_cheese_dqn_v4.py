...
epsilon_min = 0.01
explore_step = run_step * 0.08  # 천만중의 3.5퍼만 eps move = 35만
epsilon_delta = (epsilon_init - epsilon_min) / (1 * explore_step) if train_mode else 0.

# 모델 저장 및 불러오기 경로
date_time = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
save_path = f"./saved_models/full_cnn_cheese_dqn10_v4/{date_time}"
load_path = f"./saved_models/full_cnn_cheese_dqn10_v3/20231115155738"
    ...
    def to_gray(frame1, frame2=None):
        ...
        if frame2 is not None:
            new_frame = torch.sign(
                torch.sign(gray_transform(frame1))[:, 20:, 3:-3] - 0.5 * torch.sum(torch.where(frame2 > 0, frame2, torch.tensor(0.)), dim=0))
        else:
            new_frame = torch.sign(gray_transform(frame1))[:, 20:, 3:-3].float()

        new_frame[:, :60, :] = new_frame[:, :60, :]/2
        tmp1 = torch.where(new_frame == 0.5, new_frame, torch.tensor(0.))
        tmp2 = torch.where(new_frame != 0.5, new_frame, torch.tensor(0.))
        new_frame = torch.concatenate((tmp1, tmp2))
        return new_frame
...
class DQN(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = torch.nn.Conv2d(in_channels=2, out_channels=32,
...
if __name__ == '__main__':
    ...
    for step in range(run_step + test_step):
        ...
        fake_reward = real_reward * 25

        if torch.max(torch.sum(state[1, :, :], dim=0)) >= 6:  # bar와 ball이 겹치면==8, bar==4, ball==4, old object=-4
            fake_reward += 1
        if torch.min(torch.sum(state[1, :, :], dim=0)) <= -6:
            fake_reward += 1
