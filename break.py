...
class ACAgent():
    ...
    def soft_update_target(self):
        for target_param, local_param in zip(self.target_actor_critic.parameters(), self.actor_critic.parameters()):
...
#        if reward == 1:  # if ball breaks
#            reward *= 1
#        if lives != info['lives']:  # 목숨을 깔때마다 점수 마이너스
#            lives = info['lives']
#            reward -= 1 * (5-lives)
        ...
        if train_mode:
            agent.append_sample(old_state, action.data.cpu().numpy(), reward, state, [done], action_idx.to(device))
