...
save_path = f"./saved_models/R_new_cheese/{date_time}"
...
class R(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.softmax(self.fc2(x), 1)


class RAgent():
    def __init__(self):
        self.network = R().to(device)
        ...
        if load_model:
            ...
            self.network.load_state_dict(checkpoint["reinforce"])
    ...
    def save_model(self):
        ...
        torch.save({"reinforce": self.network.state_dict(), "optimizer": self.optimizer.state_dict()},
