...
save_path = f"./saved_models/full_cnn_cheese_dqn10_v3/{date_time}"
...
if __name__ == '__main__':
    ...
    for step in range(run_step + test_step):
        ...
        if torch.max(torch.sum(state[:, 60:, :], dim=1)) >= 6:  # bar와 ball이 겹치면==8, bar==4, ball==4, old object=-4
            fake_reward += 1
        if torch.min(torch.sum(state[:, 60:, :], dim=1)) <= -6:
            fake_reward += 1
